...
async function main() {
  console.log('🔍 Analisando prompts...\n');

  const snap = await firestore.collection('prompts').get();
  const prompts = snap.docs.map(d => ({ id: d.id, ...d.data() } as Prompt));

  const results = {
    already_correct: [] as Prompt[],
    will_update: [] as { prompt: Prompt; detected: number }[],
    no_detection: [] as Prompt[],
  };
  ...
  for (const prompt of prompts) {
    ...
    if (detected === undefined) {
      results.no_detection.push(prompt);
    } else if (prompt.peopleCount === detected) {
      results.already_correct.push(prompt);
    } else {
      results.will_update.push({ prompt, detected });
    }
  }

  // Relatório
  console.log('📊 RELATÓRIO DE DETECÇÃO\n');
  console.log(`✅ Já corretos (${results.already_correct.length}):`);
  results.already_correct.slice(0, 5).forEach(p =>
    console.log(`   - "${p.title}" → ${p.peopleCount} pessoa(s)`)
  );
  if (results.already_correct.length > 5) console.log(`   ... e mais ${results.already_correct.length - 5}`);

  console.log(`\n🔄 Serão atualizados (${results.will_update.length}):`);
  results.will_update.forEach(({ prompt, detected }) =>
    console.log(`   - "${prompt.title}" → ${prompt.peopleCount ?? 'não definido'} → ${detected} pessoa(s)`)
  );

  console.log(`\n❓ Não detectados (${results.no_detection.length}):`);
  results.no_detection.slice(0, 10).forEach(p =>
    console.log(`   - "${p.title}"`)
  );
  if (results.no_detection.length > 10) console.log(`   ... e mais ${results.no_detection.length - 10}`);

  console.log('\n─────────────────────────────────');
  console.log(`Total: ${prompts.length} prompts`);
  console.log(`✅ Já corretos: ${results.already_correct.length}`);
  console.log(`🔄 Para atualizar: ${results.will_update.length}`);
  console.log(`❓ Sem detecção: ${results.no_detection.length}`);
  console.log('─────────────────────────────────');
  console.log('\n⚠️  Nada foi salvo ainda. Revise o relatório acima.');
  console.log('Para aplicar as mudanças, rode: npm run apply-people');
}

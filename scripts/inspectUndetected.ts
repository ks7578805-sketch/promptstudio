...
async function main() {
  ...
  const undetected = prompts.filter(p => detectPeopleCount(p) === undefined);

  console.log(`📋 Total não detectados: ${undetected.length}\n`);
  console.log(`Mostrando 10 amostras (título + primeiras 200 chars do prompt + tags):\n`);
  console.log('═'.repeat(80));

  const sample = undetected.slice(0, 10);
  sample.forEach((p, i) => {
    const preview = (p.prompt ?? '').replace(/\s+/g, ' ').slice(0, 200);
    console.log(`\n[${i + 1}] ${p.title}`);
    console.log(`    id: ${p.id}`);
    console.log(`    tags: ${JSON.stringify(p.tags ?? [])}`);
    console.log(`    prompt: ${preview}${(p.prompt ?? '').length > 200 ? '…' : ''}`);
    console.log('─'.repeat(80));
  });
}
